...
(function(){
    ...
    function (item, text) <>{item.item.url ? <a highlight="URL" href={item.item.url}>{item.item.url}</a> : <span>{text}</span>}</>
    ];
                    ...
                    if (q.length < 3) {
                      ...
                      context.completions = p;
                        ...
                        context.completions = [{text: path, desc: "current path", index: ""}]
                            .concat([{text: path + a, desc: b, index: ""} for ([a, b] in Iterator(tags))], bm);
                    }
                } catch (ex) {
